...
public class AddNonReplaceableBlocksGUI extends BaseGUI<ConfigGUI> {
    ...
    private JPanel leftPanel(JFrame frame, ConfigGUI prevGUI) {
        ...
        List<IBlockState> blockStates = ((SortedListModel<IBlockState>) prevGUI.nonReplaceableBlockList.getModel()).getElements();
        nonConfigBlocks = new JList<>(new SortedListModel<>(registry.getKeys().stream().map(registry::getObject).map(Block::getDefaultState).filter(blockState -> !blockStates.contains(blockState)).collect(Collectors.toList()), new IBlockStateIDComparator()));
        ...
        test = new JScrollPane(nonConfigBlocks);
        panel.add(test, gbc(0, 1));
    ...
    private JPanel rightPanel(JFrame frame, ConfigGUI prevGUI) {
        ...
        configBlocks = new JList<>(new SortedListModel<>(((SortedListModel<IBlockState>) prevGUI.nonReplaceableBlockList.getModel()).getElements(), new IBlockStateIDComparator()));
    ...
    private void transfer(JFrame frame, JList<IBlockState> from, JList<IBlockState> to) {
        ...
        from.setModel(new SortedListModel<>(fromModel.getElements(), new IBlockStateIDComparator()));
        from.validate();
        to.setModel(new SortedListModel<>(blockStates, new IBlockStateIDComparator()));
        to.validate();
        frame.pack();
    }
    ...
    protected final void update(ConfigGUI configGUI) {
        SortedListModel<IBlockState> model = (SortedListModel<IBlockState>) configGUI.nonReplaceableBlockList.getModel();
        model.clear();
