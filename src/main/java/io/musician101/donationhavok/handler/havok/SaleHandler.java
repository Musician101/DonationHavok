...
class SaleHandler {
    ...
    public void startSale(double discount, int saleLength) {
        ...
        broadcast(new TextComponentString("A sale has been triggered! All rewards are now ").setStyle(new Style().setColor(TextFormatting.WHITE)).appendSibling(new TextComponentString((discount * 100) + "% off").setStyle(new Style().setColor(TextFormatting.AQUA)))
                .appendSibling(new TextComponentString(" for the next ").setStyle(new Style().setColor(TextFormatting.WHITE)))
                .appendSibling(new TextComponentString((saleLength / 20) + " seconds!").setStyle(new Style().setColor(TextFormatting.AQUA))));
    ...
    public void onTick(ServerTickEvent event) {
        if (event.side == Side.SERVER && event.type == TickEvent.Type.SERVER && event.phase == Phase.START) {
            if (saleLength != startLength) {
                ITextComponent message = new TextComponentString("The current sale will end in ").setStyle(new Style().setColor(TextFormatting.WHITE))
                        .appendSibling(new TextComponentString(saleLength / 20 + " seconds.").setStyle(new Style().setColor(TextFormatting.AQUA)));
                switch (saleLength) {
                    case 100:
                    case 200:
